  ...
  <div class="review-page">
    ...
    <el-card v-if="result" class="result-card">
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="审核时间">{{ result.timestamp }}</el-descriptions-item>
      ...
      <div class="result-reason">
        <el-alert :title="result.reason" :type="resultType" :closable="false" show-icon />
      </div>
      ...
      <div v-if="result.hit_rules && result.hit_rules.length > 0" class="hit-rules">
        ...
        <el-table :data="result.hit_rules" border stripe>
          ...
          <el-table-column prop="keywords" label="命中关键词">
            <template #default="{ row }">
              <el-tag 
...
const resultType = computed(() => {
  ...
  return types[result.value.result] || 'info'
})

const resultText = computed(() => {
  if (!result.value) return ''
  const texts = { pass: '放行', manual: '转人工', block: '拦截' }
  return texts[result.value.result] || result.value.result
})
