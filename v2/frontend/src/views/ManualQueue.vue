  ...
  <div class="manual-queue">
    ...
    <el-card>
      ...
      <el-table :data="tasks" border stripe v-loading="loading">
        ...
        <el-table-column prop="machine_decision" label="机器建议" width="100">
          <template #default="{ row }">
            <el-tag :type="decisionType(row.machine_decision)">
              {{ decisionText(row.machine_decision) }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" title="人工审核" width="600px">
      <div v-if="currentTask" class="review-dialog">
        <el-alert
          :title="`机器建议：${decisionText(currentTask.machine_decision)}（分数：${Math.round(currentTask.machine_score * 100)}%）`"
